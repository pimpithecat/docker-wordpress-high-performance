...
init_base_structure() {
  ...
  # Ensure sites directory has correct ownership for user operations
  if [[ -d "$SITES_DIR" ]]; then
    sudo chown $(id -u):$(id -g) "$SITES_DIR" 2>/dev/null || chown $(id -u):$(id -g) "$SITES_DIR"
  fi
...
process_domain() {
  ...
  if [[ "${DRY_RUN}" == false ]]; then
    ...
    # Generate wp-config.php manually (WordPress image entrypoint unreliable)
    DB_PASSWORD_VALUE=$(cat "$SECRETS_DIR/db_password.txt")
    cat > "$SITE_DIR/wordpress/wp-config.php" <<WPCONFIG
<?php
/**
 * WordPress Configuration - Auto-generated by setup.sh
 * Site: ${DOMAIN}
 */
...
WPCONFIG
    ...
    log "  ✓ wp-config.php generated with Redis config"
  fi
...
FLUSH PRIVILEGES;
EOF
      log "  ✓ Database entry added to init script"
    else
      log "  ✓ Database entry already exists in init script"
    fi
  fi

  # 3b) Execute database creation immediately (always run if DB is up)
  if [[ "${DRY_RUN}" == false ]]; then
    if docker compose -f "$COMPOSE_FILE" ps db 2>/dev/null | grep -q Up; then
      DB_ROOT_PASS=$(cat "$SECRETS_DIR/db_root_password.txt")
      log "  Creating database ${DB_NAME}..."
      docker compose -f "$COMPOSE_FILE" exec -T db mysql -u root -p"${DB_ROOT_PASS}" \
        -e "CREATE DATABASE IF NOT EXISTS ${DB_NAME} CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;" \
        -e "GRANT ALL PRIVILEGES ON ${DB_NAME}.* TO 'wp_user'@'%';" \
        -e "FLUSH PRIVILEGES;" 2>&1 | grep -v "Warning" || true
      log "  ✓ Database ${DB_NAME} created/verified"
    else
      warn "  DB container not running, will create on next startup"
    fi
  fi
...
cmd_add() {
  ...
  # Ensure sites directory is writable by current user
  if [[ -d "$SITES_DIR" ]]; then
    sudo chown $(id -u):$(id -g) "$SITES_DIR" 2>/dev/null || true
  fi
...
cmd_remove() {
  ...
  if [[ "$TARGET" == "--all" ]]; then
    ...
    # Drop all WordPress databases
    if docker compose -f "$COMPOSE_FILE" ps db 2>/dev/null | grep -q Up; then
      DB_ROOT_PASS=$(cat "$SECRETS_DIR/db_root_password.txt")
      log "Dropping all WordPress databases..."
      for site in "${EXISTING_SITES[@]}"; do
        SHORT=$(short_name "$site")
        docker compose -f "$COMPOSE_FILE" exec -T db mysql -u root -p"${DB_ROOT_PASS}" \
          -e "DROP DATABASE IF EXISTS wp_${SHORT};" 2>&1 | grep -v "Warning" || true
      done
      log "  ✓ All databases dropped"
    fi

    # Flush all Redis caches
    log "Flushing all Redis caches..."
    for site in "${EXISTING_SITES[@]}"; do
      SHORT=$(short_name "$site")
      if docker compose -f "$COMPOSE_FILE" ps redis_${SHORT} 2>/dev/null | grep -q Up; then
        docker compose -f "$COMPOSE_FILE" exec -T redis_${SHORT} redis-cli FLUSHALL 2>/dev/null || true
      fi
    done
    log "  ✓ All Redis caches flushed"

    # Clear all nginx cache
    if docker compose ps nginx 2>/dev/null | grep -q Up; then
      log "Clearing all nginx cache..."
      docker compose exec -T nginx find /var/cache/nginx -type f -delete 2>/dev/null || true
      docker compose exec -T nginx nginx -s reload 2>/dev/null || true
      log "  ✓ All nginx cache cleared"
    fi
...
EOF
  ...
  # Clear nginx cache for this site
  if docker compose ps nginx 2>/dev/null | grep -q Up; then
    log "Clearing nginx cache for ${SHORT}..."
    docker compose exec -T nginx find /var/cache/nginx/${SHORT} -type f -delete 2>/dev/null || true
    docker compose exec -T nginx nginx -s reload 2>/dev/null || true
...
# Drop database from MySQL
  if docker compose -f "$COMPOSE_FILE" ps db 2>/dev/null | grep -q Up; then
    ...
    docker compose -f "$COMPOSE_FILE" exec -T db mysql -u root -p"${DB_ROOT_PASS}" \
      -e "DROP DATABASE IF EXISTS ${DB_NAME};" 2>&1 | grep -v "Warning" || true
    log "  ✓ Database ${DB_NAME} dropped"
  fi

  # Flush Redis cache
  if docker compose -f "$COMPOSE_FILE" ps redis_${SHORT} 2>/dev/null | grep -q Up; then
    log "  Flushing Redis cache for ${SHORT}..."
    docker compose -f "$COMPOSE_FILE" exec -T redis_${SHORT} redis-cli FLUSHALL 2>/dev/null || true
  ...
  # Store removed domain info before rebuilding arrays
  ...
  log "✓ Site $REMOVED_DOMAIN removed successfully"
}
